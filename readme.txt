...
The shortcode now supports changing the image size on a per-slideshow basis. Use the size attribute in the shortcode like this:
...
One common mistake is that to insert the images into the post using the content editor. This is not necessary--the plugin detects all images attached to the post and creates the slideshow automatically. 

Q: Does the plugin support images not uploaded via the media uploader?
