...
By default, the slideshow will use the large version of the images that WordPress automatically creates when you upload an image. You can change this default in the Settings panel or on a per-slideshow basis.

The shortcode now supports changing the image size on a per-slideshow basis, just like the built-in gallery. The image sizes available are  "thumbnail", "medium", "large", and "full". Use the size attribute in the shortcode like this:
...
A: By default, the slideshow uses the large-size images that are generated by WordPress when you upload an image. You can change this default in the settings panel for the plugin, or on a per-page basis using the size attribute (see installation instructions).
